...
public class Tank2 extends Picture implements Moveable,Hitable,Destroyedable,Blockable{
//    private int x;
//    private int y;
//    private int width;
//    private int height;
    ...
    public void setBadDirection(Direction badDirection) {
...
//        this.y = y;
        super(x,y);
        try {
            int[] size = DrawUtils.getSize("tankwar\\res\\img\\enemy_1_u.gif");
            this.width = size[0];
            this.height = size[1];
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void draw(){
        ...
        switch(this.direction) {
            case UP:
                try {
                    DrawUtils.draw("tankwar\\res\\img\\enemy_1_u.gif", x, y);
                } catch (IOException e) {
                    e.printStackTrace();
                }
                break;
            case DOWN:
                try {
                    DrawUtils.draw("tankwar\\res\\img\\enemy_1_d.gif", x, y);
                } catch (IOException e) {
                    e.printStackTrace();
                }
                break;
            case LEFT:
                try {
                    DrawUtils.draw("tankwar\\res\\img\\enemy_1_l.gif", x, y);
                } catch (IOException e) {
                    e.printStackTrace();
                }
                break;
            case RIGHT:
                try {
                    DrawUtils.draw("tankwar\\res\\img\\enemy_1_r.gif", x, y);
                } catch (IOException e) {
                    e.printStackTrace();
                }
                break;
...
//        }
    ...
    public Bullet2 shot(){
        ...
        Bullet2 bullet = new Bullet2(this);
        return bullet;
    }
    ...
    public boolean checkHit(/*Steel steel*/ Blockable b){
        //  两个敌方坦克不能相互重叠,让敌我坦克都实现具有阻挡能力的接口,
        //  注意自己不能跟自己发生阻挡,如果阻挡也是自己返回false不做碰撞
        if (b == this){
            return false;
        }
        Picture p = (Picture) b;
